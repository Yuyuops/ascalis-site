...
const tools: Tool[] = [
  ...
  {
    href: "matrice-priorisation.html",
  ...
  {
    href: "diagnostic-dirigeant.html",
  ...
  return (
    <>
      ...
      <Section id="approche" label="Trois piliers, un objectif : vous rendre autonome" variant="warm">
        <SectionHeading
          tag="Approche"
          title="Trois piliers, un objectif : vous rendre autonome"
          description="Mon objectif est de me rendre inutile. Chaque mission se termine par un transfert de compétences et des outils que vos équipes maîtrisent."
        />
      ...
      <Section label="Des résultats mesurables, pas des promesses" variant="dark">
        <SectionHeading
          ...
          description="Les chiffres issus des missions terrain."
          inverse
        />
      ...
      <Section id="offres" label="Huit offres, une logique terrain" variant="light">
        <SectionHeading
          tag="Offres"
          title="Huit offres, une logique terrain"
          description="Chaque offre part d'une douleur identifiée et débouche sur un résultat mesurable. Elles sont modulaires et combinables."
        />
          ...
          <div className="divide-y divide-border">
            {offers.map((offer, index) => (
              ...
              >
                ...
                <div>
                  <h3 className="font-heading text-lg text-primary">{offer.title}</h3>
                </div>
                <p className="text-sm leading-7 text-muted-foreground">{offer.pitch}</p>
                <div>
                  <div className="text-xs uppercase tracking-[0.14em] text-muted-foreground lg:hidden">Durée</div>
                  <div className="font-heading text-sm text-primary">{offer.duration}</div>
                </div>
                <div>
                  <div className="text-xs uppercase tracking-[0.14em] text-muted-foreground lg:hidden">Prix</div>
                  <div className="font-heading text-sm text-primary">{offer.price}</div>
                </div>
                <div className="flex items-start lg:justify-end">
                  {offer.badge ? <Badge variant="priority">{offer.badge}</Badge> : <span className="text-sm text-muted-foreground">—</span>}
                </div>
              </motion.article>
            ))}
          </div>
      ...
      <Section label="Une logique de progression" variant="warm">
        <SectionHeading
          tag="Parcours"
          title="Une logique de progression"
          description="Deux portes d'entrée, un chemin vers l'autonomie."
        />
            ...
            <React.Fragment key={step.step}>
              <Reveal reduceMotion={reduceMotion} delay={index * 0.06} className="w-full lg:flex-1">
                <article className="rounded-[1.5rem] border border-border bg-white px-6 py-6 text-center shadow-[0_8px_24px_rgba(15,26,46,0.04)]">
                  <div className="mx-auto flex size-12 items-center justify-center rounded-full bg-primary font-display text-2xl text-accent-light">
                    {step.step}
                  </div>
              ...
              {index < pathwaySteps.length - 1 ? (
                <div className="font-display text-3xl text-accent lg:self-center" aria-hidden="true">
                  →
                </div>
              ) : null}
            </React.Fragment>
        ...
        <div className="grid gap-10 md:grid-cols-[300px_minmax(0,1fr)] md:items-start">
          ...
          <Reveal reduceMotion={reduceMotion} delay={0.08} className="space-y-6">
            <div>
              ...
              <p className="mt-2 font-heading text-sm uppercase tracking-[0.16em] text-accent">
                Consultant Amélioration Continue & Performance Qualité
              </p>
            </div>
            <p className="text-base leading-8 text-muted-foreground">
              Ingénieur qualité de formation, j'ai passé plus de 8 ans sur le terrain — en atelier, sur les lignes de production, au contact des opérateurs et des responsables qualité. J'interviens dans l'industrie manufacturière, le BTP et le transport & logistique.
            </p>
            <div className="rounded-[1.5rem] border-l-[3px] border-accent bg-surface-alt p-6">
              ...
              <p className="mt-3 text-sm leading-7 text-muted-foreground">
                Vous travaillez avec moi directement, pas avec un junior envoyé par un cabinet. Pas de couche commerciale, pas de rapport de 80 pages. Un interlocuteur unique qui connaît votre terrain et qui s'engage sur des résultats. Mon objectif : me rendre inutile en 6 à 12 mois.
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <span key={tag} className="rounded-full border border-border bg-surface-alt px-3 py-1 font-heading text-xs text-muted-foreground">
                  {tag}
                </span>
              ))}
            </div>
            ...
            <Reveal key={reference.author} reduceMotion={reduceMotion} delay={index * 0.06}>
              <article className="h-full rounded-[1.5rem] border border-border bg-white p-7 shadow-[0_8px_24px_rgba(15,26,46,0.04)]">
                ...
                <div className="mt-5">
                  ...
                  <p className="mt-3 inline-flex rounded-full bg-accent/10 px-3 py-1 font-heading text-xs text-accent">
                    {reference.result}
                  </p>
                  <div className={`mt-3 inline-flex rounded-full px-3 py-1 font-heading text-xs ${reference.badgeClass}`}>
                    {reference.badge}
                  </div>
                </div>
              </article>
      ...
      <Section id="secteurs" label="Les PME et ETI industrielles qui veulent performer" variant="dark">
        <SectionHeading
          tag="Pour qui"
          title="Les PME et ETI industrielles qui veulent performer"
          description="10 à 250 personnes, une production à tenir, des exigences clients qui montent. Je travaille avec des entreprises qui font des choses concrètes."
          inverse
        />
        <div className="grid gap-6 lg:grid-cols-3">
      ...
      <Section id="outils" label="Évaluez, mesurez, priorisez" variant="dark">
        <SectionHeading
          tag="Boîte à outils gratuite"
          title="Évaluez, mesurez, priorisez"
          description="10 outils interactifs pour faire le point sur votre performance qualité et votre gestion. Gratuits, sans inscription, résultats immédiats."
          inverse
        />
          ...
          {tools.map((tool, index) => {
            ...
            return (
              <Reveal key={tool.title} reduceMotion={reduceMotion} delay={index * 0.04}>
                <a
                  href={tool.href}
                  className="group block min-h-[44px] rounded-[1.5rem] border border-white/10 bg-white/5 p-6 transition-all hover:-translate-y-1 hover:border-accent/40 hover:bg-white/[0.08] focus-visible:outline focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-accent"
                >
                  ...
                  <span className="mt-4 inline-flex items-center gap-2 font-heading text-xs uppercase tracking-[0.14em] text-accent-light">
                    {tool.cta}
        ...
        <Reveal reduceMotion={reduceMotion} className="mx-auto max-w-4xl rounded-[2rem] border border-border bg-surface-alt p-6 shadow-[0_12px_32px_rgba(15,26,46,0.05)] md:p-10">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <p className="mt-4 text-base leading-8 text-muted-foreground">
              Un échange de 30 minutes pour comprendre votre contexte, identifier les leviers et voir si on peut travailler ensemble.
            </p>
          </div>
          <form className="mt-8 grid gap-4 text-left md:grid-cols-2" aria-label="Formulaire de contact">
            <Field label="Nom" id="f-name">
              <input type="text" id="f-name" placeholder="Votre nom" autoComplete="name" required className={inputClass} />
            </Field>
            <Field label="Entreprise" id="f-company">
              <input type="text" id="f-company" placeholder="Nom de l'entreprise" autoComplete="organization" className={inputClass} />
            </Field>
            <Field label="Email" id="f-email">
              <input type="email" id="f-email" placeholder="votre@email.com" autoComplete="email" required className={inputClass} />
            </Field>
            <Field label="Secteur" id="f-sector">
              <select id="f-sector" className={inputClass} defaultValue="">
            ...
            <Field label="Votre problématique en quelques mots" id="f-message" className="md:col-span-2">
              <textarea
                id="f-message"
                placeholder="Ex: On a un audit EN 9100 dans 4 mois et on ne sait pas où on en est..."
                className={`${inputClass} min-h-28 resize-y`}
              />
            </Field>
            <div className="md:col-span-2">
              <Button type="submit" variant="copper" className="w-full justify-center">
...
function Field({
  ...
  return (
    <div className={className}>
      <label htmlFor={id} className="mb-1.5 block font-heading text-xs font-medium text-muted-foreground">
        {label}
      </label>
      {children}
    </div>
...
function SectionHeading({
  ...
  return (
    <div className="mx-auto mb-14 max-w-3xl text-center">
      ...
      <h2 className={`font-display text-[clamp(2rem,4vw,3.25rem)] leading-tight ${inverse ? "text-white" : "text-primary"}`}>
        {title}
      </h2>
      {description ? (
        <p className={`mx-auto mt-4 max-w-2xl text-base leading-8 ${inverse ? "text-white/60" : "text-muted-foreground"}`}>
          {description}
        </p>
      ) : null}
    </div>
...
function GearDivider({
  ...
  return (
    <div ref={ref} className={`relative flex h-20 items-center justify-center overflow-hidden ${backgroundClassName ?? "bg-background"}`} aria-hidden="true">
      ...
      <motion.svg style={{ rotate: rotate1 }} viewBox="0 0 48 48" className="absolute left-[calc(50%-54px)] size-12 opacity-15">
        <GearPath fill={gearPrimary} centerFill={centerFill} />
      </motion.svg>
      <motion.svg style={{ rotate: rotate2 }} viewBox="0 0 48 48" className="absolute left-[calc(50%+10px)] size-10 opacity-15">
        <GearPath fill={gearSecondary} centerFill={centerFill} />
      </motion.svg>
    </div>
  );
}
...
function GearPath({ fill, centerFill }: { fill: string; centerFill: string }) {
  return (
    <>
      <path
        d="M24 6l3 4.5a12 12 0 0 1 4.2 1.7l5.3-1.2 1.5 4.5-4 3.5a12 12 0 0 1 1 4.5l4.5 3-1.5 4.5-5.3-.5a12 12 0 0 1-3.2 3.2l.5 5.3-4.5 1.5-3-4.5a12 12 0 0 1-4.5-1l-3.5 4-4.5-1.5 1.2-5.3a12 12 0 0 1-1.7-4.2L6 24l1.5-4.5 5.3 1.2a12 12 0 0 1 3.2-3.2L15.5 12l4.5-1.5z"
        fill={fill}
      />
...
const inputClass =
  "w-full rounded-xl border border-border bg-background px-4 py-3 text-base text-foreground outline-none transition focus:border-accent focus-visible:outline focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-accent";
